...
public class SoundEventHandler
{
    ...
    public void onClientTick(ClientTickEvent event)
    {
        			...
        			)
        	{
    	        ...
    	        for (int xScan = MathHelper.ceiling_double_int(player.posX-beaconSoundRadius); xScan <= MathHelper.floor_double(player.posX+beaconSoundRadius); xScan++)
    	        {
    	        	...
    	        	for (int yScan = MathHelper.ceiling_double_int(player.posY-MathHelper.sqrt_double(beaconRadiusSqrd-yzCircleDist*yzCircleDist)); yScan <= MathHelper.floor_double(player.posY+MathHelper.sqrt_double(beaconRadiusSqrd-yzCircleDist*yzCircleDist)); yScan++)
    	        	{
    	        		...
    	        		{
    	        			TileEntity tileentity = world.getTileEntity(new BlockPos(xScan, yScan, zScan));
    	        					...
    	        					)
    	        			{
    	        				...
    	        				boolean beaconThinksItIsOn = teBeacon.getField(0) > 0; // 0  is levels, 1 and 2 are primary and secondary effect
    	        				boolean beaconIsOnBruteFrc = false; 
    	        				
    	        				// Scan the blocks underneath and determine whether the beam SHOULD be on
    	        				int j = 1;
            	                int yBaseScan = teBeacon.getPos().getY() - j;
            	                ...
            	                for (int xBaseScan = teBeacon.getPos().getX() - j; xBaseScan <= teBeacon.getPos().getX() + j && flag; ++xBaseScan)
            	                {
            	                    for (int zBaseScan = teBeacon.getPos().getZ() - j; zBaseScan <= teBeacon.getPos().getZ() + j; ++zBaseScan)
            	                    {
            	                        Block block = world.getBlockState(new BlockPos(xBaseScan, yBaseScan, zBaseScan)).getBlock();

            	                        if (!block.isBeaconBase(world, new BlockPos(xBaseScan, yBaseScan, zBaseScan), teBeacon.getPos()))
            	                        {
    	...
    	if (event.phase == Phase.END)
    	{
				...
				{ 
            		...
            		if (codedPosition <= (totalBlocks-1)) // Occasionally, a position will be outside the cube, and that's fine. Just reject it.
            		{
            			...
            			if (
            					...
            					&& world.getBlockState(new BlockPos(fx, fy, fz)).getBlock() == Blocks.lit_furnace // Is a lit furnace
            					)
            			{
            				world.playSound(
          						  fx + 0.5
    				...
    				)
    		{
        		...
        		ContainerEnchantment container_reflected = ReflectionHelper.getPrivateValue(GuiEnchantment.class, gui, new String[]{"field_147075_G", "container"});
        		
        		if (this.itemInSlot == null) // Dummy item is null
        		{
        			if (container_reflected.tableInventory.getStackInSlot(0) != null) // but there is something in the slot
        			{
        				// Update the dummy item to match what's in the slot
        				this.itemInSlot = container_reflected.tableInventory.getStackInSlot(0);
        			}
        		}
        		else // The Dummy item is something
        		{
        			if (container_reflected.tableInventory.getStackInSlot(0) == null) // the  slot is empty
        			{
        				// The item was taken out of the slot.
        				this.itemInSlot = null;
        			}
        			else if (
        						!ItemStack.areItemStacksEqual(this.itemInSlot, container_reflected.tableInventory.getStackInSlot(0)) // Slot stack has changed
        					) // The slot item is different now
        			{
        				if (
        						!this.itemInSlot.isItemEnchanted() && container_reflected.tableInventory.getStackInSlot(0).isItemEnchanted() // to something enchanted
        						&& this.itemInSlot.getItem() == container_reflected.tableInventory.getStackInSlot(0).getItem() // But the item is the same
        						&& this.itemInSlot.getItemDamage() == container_reflected.tableInventory.getStackInSlot(0).getItemDamage() // with the same damage
        						)
        				{
    ...
    public void onPlaySound(PlaySoundEvent event)
    {
    	...
    	{
    		LogHelper.info(event.sound.getSoundLocation().getResourceDomain()+":"+event.name
    				+ ", volume: " + event.sound.getVolume()
    				+ ", pitch: " + event.sound.getPitch()
    				+ ", at block " + Minecraft.getMinecraft().theWorld.getBlockState(new BlockPos(MathHelper.floor_float(event.sound.getXPosF()), MathHelper.floor_float(event.sound.getYPosF()), MathHelper.floor_float(event.sound.getZPosF()))).getBlock()	
		...
		if (
				...
				&& event.sound.getSoundLocation().getResourceDomain().equals("minecraft")
				)
		{
			...
			while(itr.hasNext())
			{
				...
				ResourceLocation resourceLocation_reflected = ReflectionHelper.getPrivateValue(PositionedSound.class, positionedSound, new String[] {"field_147664_a", "positionedSoundLocation"});
    			...
    			)
    	{
        	...
        	BlockPos pos_at_sound = new BlockPos(
        	...
        	IBlockState blockstate_at_sound = world.getBlockState(pos_at_sound);
        	Block block_at_sound = blockstate_at_sound.getBlock();
    		
    		
    		// Check vanilla sounds
    		if (event.sound.getSoundLocation().getResourceDomain().equals("minecraft"))
    		{
    			...
    			if (GeneralConfig.zombievillagerSounds)
    			{
    				...
    				List zombieList = world.getEntitiesWithinAABB(
		    				EntityZombie.class, new AxisAlignedBB(
        		...
        		if (
        				...
        				&& world.isAirBlock(pos_at_sound.up())
						...
						else
						{
							if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
							{
								if (
				        				...
				        				&& movingobjectposition.getBlockPos().getX()==MathHelper.floor_float(soundX)
				        				&& movingobjectposition.getBlockPos().getY()==MathHelper.floor_float(soundY-0.01F)
				        				&& movingobjectposition.getBlockPos().getZ()==MathHelper.floor_float(soundZ)
										)
								{
									event.result = null;
			        				return;
    			...
    			if (
        				...
        				//&& blockatps.getMaterial()==Material.wood // TODO - Check this in 1.9+ for material specifics
        		...
        		if (event.name.equals("note.harp")
        				...
        				|| (event.name.equals("note.snare")
        						&& world.getBlockState(pos_at_sound.down()).getBlock()==Blocks.soul_sand)
        				// Special handler for glowstone
        				|| (event.name.equals("note.hat")
        						&& world.getBlockState(pos_at_sound.down()).getBlock()==Blocks.glowstone)
        				// Special handler for bone // TODO - Change in 1.10
        				|| (event.name.equals("note.bd")
        						&& (
        								world.getBlockState(pos_at_sound.down()).getBlock().getClass().toString().substring(6).toLowerCase().contains("boneblock")
        								|| world.getBlockState(pos_at_sound.down()).getBlock().getClass().toString().substring(6).toLowerCase().contains("blockbone")
        								|| world.getBlockState(pos_at_sound.down()).getBlock().getClass().toString().substring(6).toLowerCase().contains("blockbopbone")
        								)
        				...
        				&& GeneralConfig.noteBlockSounds)
        		{
        			...
        			Block blockBeneath = world.getBlockState(pos_at_sound.down()).getBlock();
    			...
    			if (
    					...
    					||(block_at_sound instanceof BlockStoneSlab
    							&& (BlockStoneSlab.EnumType)blockstate_at_sound.getValue(BlockStoneSlab.VARIANT)==BlockStoneSlab.EnumType.NETHERBRICK)
    							)
	...
	public void onPlaySoundAtEntity(PlaySoundAtEntityEvent event)
	{
		if (!event.entity.worldObj.isRemote)
		{
			...
			{
				event.setCanceled(true);
				event.entity.worldObj.playSoundAtEntity(event.entity, Reference.MOD_ID+":enchant.thorns", event.volume, event.pitch);
			}
	...
	public void onLivingUpdateEvent(LivingUpdateEvent event)
	{
		if (!event.entityLiving.worldObj.isRemote)
		{
					...
					)
			{
				...
				{
					event.entity.playSound(Reference.MOD_ID+":entity.witch.ambient", getEntityLivingVolume((EntityLivingBase)event.entity, EntityLivingBase.class), getEntityLivingPitch((EntityLivingBase)event.entity, EntityLivingBase.class));
			...
			if (event.entity instanceof EntityPlayer)
			{
	    				...
	    				)
	    		{
					...
					// Items attached to the player as a Capabilities tag
					IPlayerArmorTracker ipat = player.getCapability(PlayerArmorTrackerProvider.PLAYER_ARMOR_TRACKER, null);
					
					ItemStack storedBoots = ipat.getBoots()==null? null : ItemStack.loadItemStackFromNBT(ipat.getBoots());
					ItemStack storedLeggings = ipat.getLeggings()==null? null : ItemStack.loadItemStackFromNBT(ipat.getLeggings());
					ItemStack storedChestplate = ipat.getChestplate()==null? null : ItemStack.loadItemStackFromNBT(ipat.getChestplate());
					ItemStack storedHelmet = ipat.getHelmet()==null? null : ItemStack.loadItemStackFromNBT(ipat.getHelmet());
							...
							&& (
									...
									|| (storedBoots!=null 
											&& (    // ...that's not its durability.
													...
													|| !(playerBoots.getTagCompound() == null && storedBoots.getTagCompound() != null ? false : playerBoots.getTagCompound() == null || playerBoots.getTagCompound().equals(storedBoots.getTagCompound()))
													)
											)
									)
							)
							...
							&& (
									...
									|| (storedLeggings!=null 
											&& (    // ...that's not its durability.
													...
													|| !(playerLeggings.getTagCompound() == null && storedLeggings.getTagCompound() != null ? false : playerLeggings.getTagCompound() == null || playerLeggings.getTagCompound().equals(storedLeggings.getTagCompound()))
													)
											)
									)
							)
							...
							&& (
									...
									|| (storedChestplate!=null 
											&& (    // ...that's not its durability.
													...
													|| !(playerChestplate.getTagCompound() == null && storedChestplate.getTagCompound() != null ? false : playerChestplate.getTagCompound() == null || playerChestplate.getTagCompound().equals(storedChestplate.getTagCompound()))
													)
											)
									)
							)
							...
							&& (
									...
									|| (storedHelmet!=null 
											&& (    // ...that's not its durability.
													...
													|| !(playerHelmet.getTagCompound() == null && storedHelmet.getTagCompound() != null ? false : playerHelmet.getTagCompound() == null || playerHelmet.getTagCompound().equals(storedHelmet.getTagCompound()))
													)
											)
									)
							)
	...
	public void onLivingHurtEvent(LivingHurtEvent event)
	{
		if (!event.entityLiving.worldObj.isRemote)
		{
			...
			// --- Witch hurt --- //
			if (event.entity instanceof EntityWitch && ((EntityLiving)event.entity).getHealth()-event.ammount > 0
					&& GeneralConfig.witchSounds
					)
			{
				event.entity.playSound(Reference.MOD_ID+":entity.witch.hurt", getEntityLivingVolume((EntityLivingBase)event.entity, EntityLivingBase.class), getEntityLivingPitch((EntityLivingBase)event.entity, EntityLivingBase.class));
				return;
			}
					...
					)
			{
				...
				if (targetEntity.canAttackWithItem())
				{
					if (!targetEntity.hitByEntity(playerSource))
					{
						...
						{
							enchantmentDamage = EnchantmentHelper.getModifierForCreature(playerSource.getHeldItem(), ((EntityLivingBase)targetEntity).getCreatureAttribute());//.getEnchantmentModifierLiving(playerSource, (EntityLivingBase)targetEntity);
						}
						...
						if (attackDamage > 0.0F || enchantmentDamage > 0.0F)
						{
							...
							// Knockback degree
							int i = EnchantmentHelper.getKnockbackModifier(playerSource);
							...
							boolean targetTakesDamage = !targetEntity.isEntityInvulnerable(event.source) && event.ammount > 0F;
	...
	public void onLivingDeathEvent(LivingDeathEvent event)
	{
		if (!event.entityLiving.worldObj.isRemote)
		{
			...
			{
				event.entity.playSound(
						Reference.MOD_ID+":entity.witch.death",
						getEntityLivingVolume((EntityLivingBase)event.entity, EntityLivingBase.class),
						getEntityLivingPitch((EntityLivingBase)event.entity, EntityLivingBase.class)
						);
				return;
	...
	public void onPlayerInteractEvent(PlayerInteractEvent event)
	{
		...
		if ( !world.isRemote )
		{
			...
			if (	// This has to go FIRST in line, because the sound should play no matter what's in your hand.
					isPlayerLeadingAnimal(player, world, event.pos)
					//&& itemStack != null
					//&& itemStack.getItem() == Items.lead
					&& world.getBlockState(event.pos).getBlock() instanceof BlockFence // "fence" type
			...
			if(hangingEntityClass!=null && !hangingSound.equals(""))
			{
				...
				if (event.face == EnumFacing.DOWN) {return;} // Can't be placed on the bottom
		        else if (event.face == EnumFacing.UP) {return;} // Can't be placed on the top
		        else
		        {
					BlockPos blockpos = event.pos.offset(event.face);
					
		        	EntityHanging entityhanging = (EntityHanging)
		            		(hangingEntityClass == EntityPainting.class ?
		            				new EntityPainting(world, blockpos, event.face) :
		            					(hangingEntityClass == EntityItemFrame.class ?
		            							new EntityItemFrame(world, blockpos, event.face) :
		            								null));
		            
		        	if (!player.canPlayerEdit(blockpos, event.face, itemStack)) {return;}
		        	else
		        	{
			...
			if (
					...
					&& event.action == Action.RIGHT_CLICK_BLOCK 
					&& (world.getBlockState(event.pos) == Blocks.log
						|| world.getBlockState(event.pos) == Blocks.log2)
					&& GeneralConfig.strippedlogSounds
					)
			{
				world.playSoundEffect(
					...
					)
			{
				if (
						event.action == Action.RIGHT_CLICK_BLOCK 
						&& world.getBlockState(event.pos).getBlock() == Blocks.end_portal_frame
						)
				{
					if (!(((Boolean)(world.getBlockState(event.pos)).getValue(BlockEndPortalFrame.EYE)).booleanValue()))
					{
						...
						Integer[] portalCenter = endPortalTriggering(world, event.pos);
					...
					)
		    {
				...
				if (side == EnumFacing.DOWN) {return;} // I assume 0 means "bottom"
				else if (player.canPlayerEdit(event.pos.offset(side), side, itemStack))
		        {
					IBlockState iblockstate = world.getBlockState(event.pos);
		            Block block = iblockstate.getBlock();
					
	            	if (
	            			world.isAirBlock(event.pos.up())
	            			&& (block == Blocks.grass ||
	            					(block == Blocks.dirt &&
	            					((BlockDirt.DirtType)iblockstate.getValue(BlockDirt.VARIANT)==BlockDirt.DirtType.DIRT
	            					|| (BlockDirt.DirtType)iblockstate.getValue(BlockDirt.VARIANT)==BlockDirt.DirtType.COARSE_DIRT)
	            							)
	            					)
	            			)
	            	{
	            		world.playSoundEffect(
								  event.pos.getX() + 0.5
								, event.pos.getY() + 1F // At the top face of the block
								, event.pos.getZ() + 0.5
					...
					)
			{
		        ...
		        else
		        {
		            ...
		            if (!world.canBlockBePlaced(block, blockpos, false, side, player, itemStack))
		            {
		            	// Can't put redstone here because it's not a legal spot
			            return;
		            }
					...
					)
		    {
				...
				if (side != EnumFacing.UP) {return;} // Must be top face
		        else if (player.canPlayerEdit(event.pos, side, itemStack)
		        		&& player.canPlayerEdit(event.pos.up(), side, itemStack))
		        {
		            if (world.getBlockState(event.pos).getBlock().canSustainPlant(world, event.pos, EnumFacing.UP, (IPlantable)itemStack.getItem())
		            		&& world.isAirBlock(event.pos.up()))
		            {
		            	// Mundane seeds
		            	if (world.getBlockState(event.pos).getBlock() == Blocks.farmland)
		            	{
		            		world.playSoundEffect(
		            				  event.pos.getX() + 0.5
									, event.pos.getY() + 1F // At the top face of the block
		            	...
		            	// Nether wart
		            	if (world.getBlockState(event.pos).getBlock() == Blocks.soul_sand)
		            	{
		            		world.playSoundEffect(
		            				  event.pos.getX() + 0.5
									, event.pos.getY() + 1F // At the top face of the block
					...
					)
			{
				...
				else
				{
					if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
					{
						...
						if(!player.canPlayerEdit(pos, movingobjectposition.sideHit, itemStack)) {return;}
					...
					)
			{
				...
				else
				{
					if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
					{
						...
						boolean isValidCauldron = (world.getBlockState(pos).getBlock() == Blocks.cauldron &&
								(((Integer)(world.getBlockState(pos)).getValue(BlockCauldron.LEVEL)).intValue() > 0));
						...
						if(world.getBlockState(pos).getBlock() == Blocks.water || world.getBlockState(pos).getBlock() == Blocks.flowing_water) {isWater = true;}
			...
			if (
					...
					&& (
							...
							//|| itemStack.getItem() == Items.bucket // TODO - Can extract water in 1.9+
							)
				...
				else
				{
					if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
					{
						...
						boolean isValidCauldron = (
								world.getBlockState(pos).getBlock() == Blocks.cauldron
								&& (bucketIsFull ? ((Integer)(world.getBlockState(pos)).getValue(BlockCauldron.LEVEL)).intValue() <3 : ((Integer)(world.getBlockState(pos)).getValue(BlockCauldron.LEVEL)).intValue()==3)
								);
	...
	public void onFillBucketEvent(FillBucketEvent event)
	{
		...
		if (!event.world.isRemote)
		{
			...
			{
				event.world.playSoundEffect(
						  target.getBlockPos().getX() + 0.5
						, target.getBlockPos().getY() + 0.5
						, target.getBlockPos().getZ() + 0.5
			...
			if ( 
					...
					&& event.world.getBlockState(target.getBlockPos()).getBlock().getMaterial() == Material.water
					)
			{
				event.world.playSoundEffect(
							  target.getBlockPos().getX() + 0.5
			...
			if ( 
					GeneralConfig.bucketSounds &&
					event.current.getItem()==Items.bucket
					&& event.world.getBlockState(target.getBlockPos()).getBlock().getMaterial().isLiquid()
					)
			{
				event.world.playSoundEffect(
							  target.getBlockPos().getX() + 0.5
	...
	private Integer[] endPortalTriggering (World world, BlockPos pos)
	{
				...
				)
		{
			...
			IBlockState possibleFrameBlockState=null;
			...
			for (int nudge = -1; nudge <= 1; nudge++) // The frame block might not be the center of a side, so check each of the three possible offsets
			{
				...
				{
					possibleFrameBlockState = world.getBlockState(new BlockPos(
					...
					possibleFrameBlock = possibleFrameBlockState.getBlock();
					
					possibleFrameMeta = possibleFrameBlock.getMetaFromState(possibleFrameBlockState);
	 ...
	 */
    public static boolean isPlayerLeadingAnimal(EntityPlayer player, World worldIn, BlockPos fence)
    {
        ...
        for (EntityLiving entityliving : worldIn.getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB((double)i - boxRadius, (double)j - boxRadius, (double)k - boxRadius, (double)i + boxRadius, (double)j + boxRadius, (double)k + boxRadius)))
        {
